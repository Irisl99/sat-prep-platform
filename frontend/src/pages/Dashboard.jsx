...
export default function Dashboard({ user, onLogout, onStartTest }) {
  ...
  const usagePct = isPremium ? 100 : Math.min((testsUsed / freeLimit) * 100, 100);
    ...
    <div style={S.root}>
      ...
      <div style={S.body}>
        <div style={S.main}>
          ...
          <div style={{background:"#162040",border:"0.5px solid rgba(59,111,232,0.2)",borderRadius:12,padding:"20px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
            <div>
              <div style={{fontSize:10,fontWeight:500,color:"#3B6FE8",textTransform:"uppercase",letterSpacing:".08em",marginBottom:5}}>Welcome back</div>
              <div style={{fontSize:22,fontWeight:500,color:"#fff",letterSpacing:"-.3px",marginBottom:4}}>{user.name}</div>
              <div style={{fontSize:13,color:"#8892a4",lineHeight:1.5}}>
                {limitReached ? "You've used all 3 free tests this month. Upgrade to keep going." : `${freeLimit - testsUsed} test${freeLimit - testsUsed !== 1 ? "s" : ""} remaining this month.`}
              </div>
            </div>
            <button
              onClick={onStartTest}
              disabled={limitReached}
              style={{padding:"11px 20px",background:limitReached?"#1e2a40":"#3B6FE8",color:limitReached?"#4a5568":"#fff",border:"none",borderRadius:9,fontSize:14,fontWeight:500,cursor:limitReached?"not-allowed":"pointer",whiteSpace:"nowrap",flexShrink:0}}
            >
              {limitReached ? "Limit reached" : "Start test →"}
            </button>
          </div>
          ...
          <div style={S.card}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:14}}>
              {[["—","Best score"],["—","Avg accuracy"],["0","Tests done"]].map(([n,l])=>(
                <div key={l} style={S.stat}><div style={{fontSize:20,fontWeight:500,color:"#fff",lineHeight:1,marginBottom:2}}>{n}</div><div style={{fontSize:11,color:"#4a5568"}}>{l}</div></div>
              ))}
            </div>
            <div style={{height:"0.5px",background:"rgba(255,255,255,0.05)",margin:"12px 0"}}></div>
            {[["Algebra","—%",0,"#34d399"],["Reading & Writing","—%",0,"#3B6FE8"],["Geometry","—%",0,"#fbbf24"],["Advanced math","—%",0,"#f87171"]].map(([name,pct,w,color])=>(
              <div key={name} style={{marginBottom:10}}>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:4}}>
                  <span style={{color:"#8892a4"}}>{name}</span>
                  <span style={{color:"#fff",fontWeight:500}}>{pct}</span>
                </div>
                <div style={S.prog_bg}><div style={{height:"100%",width:`${w}%`,background:color,borderRadius:2}}></div></div>
              </div>
            ))}
            <div style={{fontSize:12,color:"#4a5568",marginTop:8,textAlign:"center"}}>Complete your first test to see progress</div>
          </div>

          <div style={S.card}>
            <div style={S.clabel}>Recent tests</div>
            {history.length === 0 ? (
              <div style={{textAlign:"center",padding:"20px 0",color:"#4a5568",fontSize:13}}>No tests yet. Start your first adaptive test above!</div>
            ) : (
              history.slice(0,3).map((exam,i)=>(
                <div key={i} style={{...S.hist_row, borderBottom: i===Math.min(history.length,3)-1 ? "none" : "0.5px solid rgba(255,255,255,0.05)"}}>
                  <div style={{fontSize:18,fontWeight:500,minWidth:44,color: exam.scores?.total>=1400?"#a78bfa":exam.scores?.total>=1200?"#3B6FE8":"#64748b"}}>{exam.scores?.total||"—"}</div>
                  <div style={{flex:1}}>
                    <div style={{fontSize:11,color:"#4a5568",marginBottom:1}}>{new Date(exam.completedAt).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})}</div>
        ...
        <div style={S.side}>
          ...
          <div style={{background:"#161e35",border:"0.5px solid rgba(167,139,250,0.2)",borderRadius:12,padding:"16px 18px"}}>
            ...
            {history.length === 0 ? (
              <div style={{fontSize:12,color:"#4a5568",textAlign:"center",padding:"12px 0"}}>Complete a test to get personalized recommendations.</div>
            ) : (
              [["Advanced math","Quadratics and nonlinear functions — focus on vertex form and completing the square.","#f87171"],
               ["Geometry","Circle theorems and arc-sector formulas appear on every Math module.","#f87171"],
               ["English conventions","Comma-splice fixes and modifier placement — 3 errors in last test.","#fbbf24"]].map(([t,d,c])=>(
                <div key={t} style={S.rec}>
                  <div style={{fontSize:12,fontWeight:500,color:"#e2e8f0",marginBottom:2,display:"flex",alignItems:"center",gap:5}}>
                    <div style={{width:5,height:5,borderRadius:"50%",background:c,flexShrink:0}}></div>{t}
                  </div>
          ...
          <div style={{background:"#161e35",border:"0.5px solid rgba(59,111,232,0.15)",borderRadius:12,padding:"16px 18px"}}>
            ...
            <div style={{height:4,background:"rgba(255,255,255,0.06)",borderRadius:2,overflow:"hidden",marginBottom:10}}>
              <div style={{height:"100%",width:`${usagePct}%`,background:limitReached?"#f87171":"#3B6FE8",borderRadius:2}}></div>
            </div>
            {!isPremium && (
              <>
                <div style={{fontSize:11,color:limitReached?"#f87171":"#8892a4",marginBottom:12}}>
                  {limitReached ? "Free limit reached. Resets next month." : `${freeLimit - testsUsed} test${freeLimit - testsUsed !== 1?"s":""} remaining this month.`}
                </div>
                <button style={{width:"100%",padding:9,background:"transparent",border:"0.5px solid #3B6FE8",color:"#3B6FE8",borderRadius:8,fontSize:12,fontWeight:500,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:5}}>
          ...
          <div style={S.card}>
            ...
            <div style={{fontSize:12,color:"#8892a4",marginBottom:8}}>
              {history.length === 0 ? "Complete a test to build your wrong book." : "Review incorrect questions from past tests."}
            </div>
            {history.length > 0 && (
